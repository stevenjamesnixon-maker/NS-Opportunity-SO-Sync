...
| Design Instruction config library | 1.0.0 | `lib/dsi_lib_config.js` | Every script ID and parameter of the Design Instruction feature. **Separate from `opsync_lib_config.js`** — see section 11 | Not deployed |
| Design Instruction opportunity user event | 1.0.0 | `dsi_ue_opportunity.js` | `afterSubmit` on Opportunity — creates a Design Instruction row when the sub-status moves to a creating value | Not deployed |
| Design Instruction row user event | 1.0.0 | `dsi_ue_design_instruction.js` | `beforeSubmit` and `afterSubmit` on `customrecord_cad_worklist` — design start stamp, completion gate, completion write-back to the opportunity | Not deployed |
...
with their parameters populated — see section 11. Grep the execution log for `DSI_` as well as
`OPPSYNC_` after every one of these. Scenarios **107–111 are blocked**: they exercise the Request
Design / Request Redraw buttons, which are not built in this phase (section 11, open decision 1).
They keep their numbers so the amendment that builds the buttons can run them as written.

| # | Scenario | Expected |
|---|---|---|
| 107 | ⛔ **Blocked.** Opportunity at *Awaiting Design Info*, view | Request Design button visible; Request Redraw not |
| 108 | ⛔ **Blocked.** Opportunity at *Design Complete*, view | Request Redraw visible; Request Design not |
| 109 | ⛔ **Blocked.** Any other sub-status, view | Neither button |
| 110 | ⛔ **Blocked.** Press Request Design, confirm | Sub-status = *Design Required*; one row as in 112; the sync's own log lines appear **only if the opportunity is at a qualifying `entitystatus`** — its gate exits silently otherwise |
| 111 | ⛔ **Blocked.** Press Request Redraw, confirm | Sub-status = *Redraw Required*; one row, type *Redraw*; browser lands on that row in edit mode |
| 112 | Set sub-status to *Design Required* by hand in edit mode | **One** row: type *New design*, on the 2026 form, name per section 11, customer / sales rep / project engineer / urgent copied from the opportunity. `DSI_ROW_CREATED` at audit naming the row, type and name |
| 113 | **Inline-edit** (XEDIT) the sub-status to *Design Required*, on an opportunity with **priority design ticked** and all three name fields populated | Row created with **every** copied field populated **and urgent ticked**. This proves the sparse-`newRecord` fallback — an unticked urgent box here means a sparse checkbox read as `false` rather than falling back to `oldRecord` |
...
| 118 | Set the map so a key equals the complete status, then move a sub-status to a creating value | No row; `DSI_CONFIG_OVERLAP` at error. **Every** creation is refused while the map overlaps. **Revert afterwards** |
| 119 | Opportunity with all three name fields blank | Name = `<tranid> · <type>`. **Also confirm on any populated opportunity that the three parts read as text, not as numbers** — numbers mean a field is a select and its `NAME_PARTS` entry needs `isSelect: true` |
...
| 124 | Complete a *New design* row properly | Opportunity sub-status = *Post Design Check*; priority design **unticked**; `DSI_ROW_COMPLETED` at audit. **Then record whether `OPPSYNC_` lines follow for that opportunity** — this is the test that settles open decision 2 in section 11. The brief expects the sync to run and update the sales orders (for a Won opportunity whose map carries *Post Design Check*); NetSuite documents that a user event cannot trigger another, in which case the sync does **not** run and the orders are **not** updated on this save |
...
| 131 | **Create** a new opportunity directly at *Design Required* | Row created, and its name begins with the **real opportunity number** — not blank and not *To Be Generated*. Confirms `tranid` is readable in `afterSubmit` on CREATE |
| 132 | Complete a row whose type is *New design* while `custscript_dsirow_cancelled_type` is **empty** | Gate applies and the completion **writes** — no type is treated as cancelled. `DSI_PARAMETER_MISSING` at error. Shows the behaviour behind open decision 4. **Revert afterwards** |
...
designer entering a completed date on the row**, which moves the opportunity on to be checked.

**Phase 1 builds the creation and completion paths. It does not build the two buttons** — see
open decision 1.
...
|---|---|---|---|
| `lib/dsi_lib_config.js` | Shared AMD module | — | **None.** File Cabinet upload only |
| `dsi_ue_opportunity.js` | User Event on Opportunity — `afterSubmit` | `customscript_dsi_ue_opportunity` | `customdeploy_dsi_ue_opportunity` |
...
| ~~`dsi_cs_opportunity.js`~~ | Client Script, attached by `beforeLoad` | — | **Not built** — open decision 1 |
...
4. Look the new sub-status up in `custscript_dsi_create_map`. Not a key → `DSI_NO_CREATE` at debug,
   return.
5. **Overlap check.** If any key of the map equals `custscript_dsi_complete_status` →
   `DSI_CONFIG_OVERLAP` at error, return, create nothing.
...
Every opportunity value is read through `effectiveValue()`, so an inline edit of only the
sub-status still sees the rest of the record.
...
that order — `NAME_PARTS` in `dsi_lib_config.js`. Each is included only when non-empty after
...
**`beforeSubmit`** — may write to the row, and may refuse its save:

- **Design start stamp.** A designer present after the save, none before it, and no start date →
  `custrecord_cad_design_start` = `new Date()`.
- **Completion gate**, on the save that completes the row: the designer, the area and the notes
  are all required. **Zero is an area** — the test is `isEmpty()`, never falsiness. Notes must be
  non-blank after trimming. Otherwise `DSI_INCOMPLETE` at audit, then a thrown `DSI_INCOMPLETE`
  error that shows the user *"To complete this design instruction, enter the designer, the area
  (m²) and designer notes."* **Skipped for a cancelled row.**

Not wrapped in a catch-all: a throw here is how the gate works. **The consequence is that any other
failure in `beforeSubmit` also refuses the row's save.**
...
All Free-Form Text, defined on the script record named, valued **on the deployment**. Every value
is an internal id and differs by environment. The *what does empty mean* test is section 5's.

| Script | Parameter | Holds | Empty means | Behaviour |
|---|---|---|---|---|
| `customscript_dsi_ue_opportunity` | `custscript_dsi_create_map` | `subStatusId:typeId` pairs — *Design Required* → *New design*, *Redraw Required* → *Redraw* | No row is ever created | **Fails closed** — logs, returns `{}` |
...
| | `custscript_dsi_complete_status` | The *Post Design Check* sub-status id. **Must equal** `custscript_dsirow_complete_status` | The overlap check is skipped; creation proceeds | **Does not throw** — logs at error. ⚠️ Open decision 3 |
| `customscript_dsi_ue_design_instruction` | `custscript_dsirow_complete_status` | The *Post Design Check* sub-status id | A completion writes nothing to the opportunity | **Fails closed** — logs at error, returns. Not a throw: the row has already saved |
| | `custscript_dsirow_cancelled_type` | The *Cancelled* design type id(s), comma-separated | No type is treated as cancelled — the gate applies to every row and **every completion writes** | **Does not throw** — logs at error, returns `[]`. ⚠️ Open decision 4 |

**Not defined in this phase**, because the buttons are not built: `custscript_dsi_btn_design_statuses`,
`custscript_dsi_btn_redraw_statuses`, `custscript_dsi_redraw_type`.
...
| `DSI_CREATE_FAILED` | error | Creation threw. **The opportunity still saved**; no row was created |
...
| `DSI_PARAMETER_MISSING` | error — **and thrown** for `custscript_dsi_form_id` | A parameter is unset or holds nothing usable. Names it and what that means |
...
> **Reserved — no script raises it in this phase:** `DSI_BEFORELOAD_FAILED`, for the buttons'
> `beforeLoad` when it is built.
...
- **The redraw reason, `custrecord_redraw_info`, is entered by hand.** No script reads or writes
  it.
- **Button visibility by sub-status only, no search in `beforeLoad`** — for when the buttons are
  built.
- **Both user events are separate from the sync and do not touch its files.**

### Open decisions — for the client

1. **The buttons' target sub-statuses have no source.** The brief's `beforeLoad` passes the client
   script the *Design Required* and *Redraw Required* ids *"from parameters createMap and
   redrawType"*. `custscript_dsi_create_map` holds those ids only as **keys**, with nothing to say
   which key is which: the *Redraw Required* key could be found as the one whose type is
   `custscript_dsi_redraw_type`, but no parameter names the *New design* type, so the *Design
   Required* key cannot be found at all. And the brief's own table says an empty
   `custscript_dsi_redraw_type` should only stop the redirect, **not** the sub-status write —
   which rules out deriving the redraw status from it too. Picking a key by elimination is the
   kind of derivation section 4 rejects: it breaks silently the day a third creating sub-status
   is added. **The buttons, the client script and their three parameters are therefore not built.**
   To unblock, choose one:
   - **(a)** two more parameters on `customscript_dsi_ue_opportunity` holding the two target
     sub-status ids directly — explicit, and consistent with how every other id in this repo
     arrives; **or**
   - **(b)** name each button's target as a *type*, and find its sub-status as the single map key
     carrying that type, failing closed on none or several — which needs a new-design-type
     parameter as well.
2. **The completion write may not re-run the opportunity's user events.** NetSuite documents that
   user event scripts cannot be executed by other user event scripts — *"you cannot chain user
   event scripts"*. `dsi_ue_design_instruction.js` writes the opportunity from its `afterSubmit`.
   If the documented rule holds, neither `opsync_ue_opportunity.js` nor `dsi_ue_opportunity.js`
   runs on that write: the sub-status changes, but **Post Design Check does not reach the sales
   orders until somebody next saves the opportunity**. The brief assumed the sync runs. Scenario
   124 settles it. The same rule bears on the sync's own recursion analysis in section 5 and on
   scenario 95.
3. **`custscript_dsi_complete_status` empty skips a safety check without throwing.** By the
   section 5 test that removes a restriction and should throw. Built as the brief specifies — it
   logs at error and creation proceeds. Making it throw is a one-line change in `getCompleteStatus()`
   for the opportunity script only.
4. **`custscript_dsirow_cancelled_type` empty makes every completion write.** In `afterSubmit` that
   is more writing, so by section 5 it fails open; in `beforeSubmit` it is more gating, so it fails
   closed. Built as the brief specifies — it logs at error and returns `[]`. The client has yet to
   supply the *Cancelled* value, so throwing now would stop every completion.

### Unverified in the account

| Item | Risk if wrong | Where it shows |
|---|---|---|
| The **types** of `custbody_mi_opp_fc`, `custbody_comm_area_ufh`, `custbody_mi_heat_source`, and that they apply to the **Opportunity** | A select read as a value puts an id in the name; a field not on the opportunity reads blank (section 0, trap 6) | Scenario 119. Fix in `NAME_PARTS` |
| The name field's **maximum length** — 83 assumed | Too high: saves fail on long names | Scenario 120. Fix `NAME_MAX_LENGTH` |
| **`custrecord_cad_opportunity` is a sourced field**, set directly with `setValue` | The link may not survive the save — an orphan row | Scenario 112: check the row appears on the opportunity's sublist |
| The **form's internal id** | A wrong id creates rows on the wrong form, or fails | Scenario 112 |
| The ***Cancelled*** **type id** | Not yet supplied | Scenario 126 |
...
| A **checkbox** not carried by a sparse XEDIT `newRecord` reads as empty, not `false` | Urgent copied as unticked on an inline edit of the sub-status | Scenario 113 |
| `tranid` is populated in `afterSubmit` on **CREATE** | The name starts without the opportunity number | Scenario 131 |
...
| Whether the custom record **auto-generates its name** | The built name is ignored or refused | Scenario 112 |
...
2. Upload `dsi_ue_opportunity.js` and `dsi_ue_design_instruction.js` into the **same folder** as the
   sync's scripts.
3. Create the two script records and deployments in the *Components* table.
4. **Define the five parameters on their script records and set them on the deployments before
   releasing.** `custscript_dsi_form_id` throws when unset. Check that the two complete-status
...
|---|---|---|---|
| `customdeploy_dsi_ue_opportunity` | to be set by the client | to be set by the client — Debug shows `DSI_NO_CREATE` | to be set by the client |
...
When the buttons are built, the client script attached through `form.clientScriptModulePath`
needs **no script record and no deployment** — only the File Cabinet file.
